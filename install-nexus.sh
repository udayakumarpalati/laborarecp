...
URL=$(curl -s https://help.sonatype.com/display/NXRM3/Download | html2text | grep unix.tar.gz|grep Unix |sed -e 's/>//g' -e 's/<//g' -e 's/(//g' | cut -d "|" -f2)
...
ln -s /home/nexus/nexus-3.7.1-02/bin/nexus /etc/init.d/nexus
echo "run_as_user=nexus" >/home/nexus/nexus-3.7.1-02/bin/nexus.rc
